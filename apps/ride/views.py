...
class RideViewSet(viewsets.ModelViewSet):
    ...
    def filter_queryset(self, queryset):
        ordering = self.request.query_params.get("ordering", "").lstrip("-")
        lat = self.request.query_params.get("lat")
        lng = self.request.query_params.get("lng")

        if ordering == "distance_to_pickup" and (not lat or not lng):
            from rest_framework.exceptions import ValidationError

            raise ValidationError(
                {
                    "detail": "Sorting by distance_to_pickup requires both 'lat' and 'lng' params."
                }
            )
    ...
    def get_queryset(self):
        ...
        queryset = queryset.prefetch_related(
            Prefetch(
                "ride_events", queryset=recent_events, to_attr="todays_events_cache"
            )
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Ride created successfully.", "data": response.data},
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Ride updated successfully.", "data": response.data},
            status=response.status_code,
        )
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Ride partially updated successfully.", "data": response.data},
            status=response.status_code,
        )
